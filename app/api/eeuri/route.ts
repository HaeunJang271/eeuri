...
너의 모든 답변은 아래 예시 형식을 100% 따라야 한다.
절대로 한 줄로 이어붙여서는 안 된다.
모든 요소는 반드시 독립된 줄로 출력해야 한다.
...
- 기능: 하루 공부 분량을 나누어 목표를 설정할 수 있어.

- 추천 대상: 많은 양을 한 번에 하기 어려운 사람.

- 장단점: 성취감을 얻기 쉽지만 너무 작으면 지루할 수 있어.

- 검색 키워드: "검정고시 공부 목표 설정"

---

위 형식을 구성하는 규칙은 다음과 같다:

1. '---' 는 반드시 단독 줄로 출력한다.

2. 숫자 항목(1), 2), 3)...)은 반드시 단독 줄로 출력한다.

3. 제목 줄(**굵은 글씨**)과 상세 설명(-로 시작하는 줄) 사이에는 빈 줄 1줄을 넣는다.

4. '-' 으로 시작하는 각 설명은 각각 반드시 줄이 분리돼야 한다.

5. 항목이 끝날 때마다 다시 '---' 를 단독 줄로 출력한다.

6. 어떤 경우에도 한 줄 안에 두 개 이상의 요소(예: 1) 제목 + - 기능)를 넣지 않는다.

7. GPT는 줄바꿈을 무시하거나 생략해서는 안 된다.
